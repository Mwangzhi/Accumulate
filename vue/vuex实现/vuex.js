...
class ModuleCollection {
    ...
    register(path, rawModule) {
        ...
        if (rawModule.modules) {
            forEach(rawModule.modules, (childName, modules) => {
                this.register(path.concat(childName), modules)
            })
        }
    }
}
function installMoudle(store, rootState, path, rootModule) {
    if (path.length > 0) {
        let parent = path.slice(0, -1).reduce((root, current) => {
            return root[current]
        }, rootState)
    ...
    forEach(rootModule._children, (childName, modul) => {
        installMoudle(store, rootState, path.concat(childName), modul)
    })
}
class Store {
    constructor(options) {
        ...
        installMoudle(this, state, [], this.modules.root)
